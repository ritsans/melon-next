...
- [ ] 34. フォロー統計表示コンポーネント
  - `components/follows/FollowStats.tsx`を実装する
  - フォロワー数とフォロー中の数を表示する
  - クリックでそれぞれの一覧ページに遷移する機能を実装する
  - 数値フォーマット（例: 1,234）を実装する
  - プロフィールページに統合する
  - _要件: 9（フォロー・フォロワー機能）_
  - _設計書: design.md - フォロー・フォロワー機能の設計 > UI/UXコンポーネント設計_

- [ ] 35. フォロー一覧取得のServer Actions
...
- [ ] 37. フォロワー一覧ページの実装
  - `app/(main)/profile/[username]/followers/page.tsx`を作成する
  - そのユーザーのフォロワー一覧を表示する
  - FollowListコンポーネントを使用する
  - 自分のフォロワー一覧か他人のフォロワー一覧かで表示を調整する
  - _要件: 9（フォロー・フォロワー機能）_
  - _設計書: design.md - フォロー・フォロワー機能の設計 > ページ構成_

- [ ] 38. フォロー中一覧ページの実装
  - `app/(main)/profile/[username]/following/page.tsx`を作成する
  - そのユーザーがフォロー中のユーザー一覧を表示する
  - FollowListコンポーネントを使用する
  - 自分のフォロー中一覧か他人のフォロー中一覧かで表示を調整する
  - _要件: 9（フォロー・フォロワー機能）_
  - _設計書: design.md - フォロー・フォロワー機能の設計 > ページ構成_
...
- [ ] 40. FeedTabsコンポーネントの実装
  - `components/posts/FeedTabs.tsx`を実装する
  - 「すべて」と「フォロー中」のタブ切り替えを実装する
  - 選択中のタブをハイライト表示する
  - タブ切り替え時のスムーズな遷移を実装する
  - _要件: 9（フォロー・フォロワー機能）_
  - _設計書: design.md - フォロー・フォロワー機能の設計 > UI/UXコンポーネント設計_

- [ ] 41. ホームフィードへのタブ機能統合
  - `app/(main)/home/page.tsx`にFeedTabsを追加する
  - タブ切り替えによる投稿フィルタリングを実装する
  - フォロー中タブでフォロー中のユーザーの投稿のみを表示する
  - フォロー中のユーザーがいない場合のメッセージを表示する
  - _要件: 9（フォロー・フォロワー機能）_
  - _設計書: design.md - フォロー・フォロワー機能の設計 > ページ構成_
...
以下のタスクは、フォロー機能完成後に実装を検討する追加機能です。
