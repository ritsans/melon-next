...
以下のタスクは、MVP完成後に実装を検討する追加機能です。投稿機能の基本（テキスト投稿、タグ付け、投稿一覧表示、リアクション機能など）が十分に安定してから実装します。

### リプライ（返信）機能（Phase 2）

MVP完成後に実装する投稿へのリプライ機能です。Twitter/Instagramのように投稿カードの下にリプライが表示されます。

- [ ] 22. データベーススキーマ更新（リプライ機能）
...
- [ ] 14. Supabase Storage設定
  - Supabase Storageで`post-images`バケットを作成する
  - RLSポリシーを設定する（投稿者のみ削除可能）
  - 公開URLアクセスを許可する設定を行う
  - _要件: 2（画像投稿機能）_

- [ ] 15. 画像アップロードコンポーネント
  - ImageUploaderコンポーネントを実装する
...
- [ ] 16. 画像バリデーションとリサイズ
  - クライアントサイドでの画像バリデーション（ファイルサイズ、形式、枚数）を実装する
  - 画像の自動リサイズ機能を実装する（最大幅1200px）
  - 画像最適化処理を実装する
  - _要件: 2（画像投稿機能）_

- [ ] 17. 投稿フォームへの画像機能統合
  - PostFormコンポーネントに画像アップロード機能を統合する
  - 投稿作成時の画像URLの保存処理を実装する
  - 投稿編集時の画像管理機能を実装する
  - _要件: 2（画像投稿機能）_

- [ ] 18. 画像表示コンポーネント
  - ImageGalleryコンポーネントを実装する（グリッド表示）
  - PostCardコンポーネントに画像グリッド表示を統合する
  - レスポンシブな画像表示を実装する
  - _要件: 2（画像投稿機能）_

- [ ] 19. 画像拡大表示機能
  - ImageLightboxコンポーネントを実装する
  - 画像クリック時のライトボックス表示を実装する
  - 画像のスワイプ/矢印ナビゲーションを実装する
  - _要件: 2（画像投稿機能）_

- [ ] 20. データベーススキーマ更新
  - postsテーブルに`image_urls`カラム（JSONB）を追加するマイグレーションを作成する
  - TypeScript型定義を更新する（Post型に`image_urls?: string[]`を追加）
  - _要件: 2（画像投稿機能）_

- [ ] 21. 画像投稿機能のテストとデプロイ
  - 画像アップロード機能の統合テストを実行する
  - 画像表示機能のE2Eテストを実行する
  - ストレージ容量の監視設定を行う
