...
**ユーザーストーリー:** ユーザーとして、他のユーザーをフォローしたい。そうすることで、興味のあるユーザーの投稿を優先的に確認でき、よりパーソナライズされたコミュニティ体験ができる。

#### 受け入れ基準

1. ユーザーがログインしている時、システムは他のユーザーをフォローする機能を提供する
2. システムは、プロフィールページにフォローボタンを表示する
3. ユーザーがフォローボタンをクリックした時、システムはフォロー関係をデータベースに保存する
4. ユーザーが既にフォローしている場合、システムはフォロー解除ボタンを表示する
5. システムは、自分自身をフォローできないように制限する
6. システムは、フォロワー数とフォロー中の数をプロフィールページに表示する
7. システムは、フォロワー一覧とフォロー中一覧を表示する機能を提供する
8. システムは、ユーザーがフォローされた時に通知を送る
9. システムは、相互フォロー状態を識別し表示する
10. システムは、フォロー中のユーザーの投稿を優先的に表示するフィード機能を提供する

#### フォロー機能の詳細

**基本フォロー機能（Phase 1）:**
1. システムは、フォロー/フォロー解除のトグル機能を提供する
2. システムは、フォロー状態をリアルタイムで反映する
3. システムは、フォロー数とフォロワー数のカウント表示を提供する
4. システムは、フォロー関係のデータベース整合性を保証する

**フォロー一覧表示（Phase 2）:**
1. システムは、プロフィールページにフォロワー一覧タブを提供する
2. システムは、プロフィールページにフォロー中一覧タブを提供する
3. システムは、各一覧でユーザー情報（アバター、表示名、ユーザー名、フォロー状態）を表示する
4. システムは、一覧からプロフィールへの遷移を可能にする
5. システムは、一覧から直接フォロー/フォロー解除できる機能を提供する

**フィード統合（Phase 3）:**
1. システムは、ホームフィードに「フォロー中」タブを追加する
2. システムは、フォロー中のユーザーの投稿のみを表示するフィルタリング機能を提供する
3. システムは、「すべて」タブですべてのユーザーの投稿を表示する
4. システムは、フォロー中のユーザーがいない場合に適切なメッセージを表示する

**通知機能（Phase 4）:**
1. システムは、ユーザーがフォローされた時に通知を作成する
2. システムは、通知テキストを「〇〇さんがあなたをフォローしました」形式で生成する
3. システムは、通知からフォロワーのプロフィールに遷移できる機能を提供する
4. システムは、相互フォロー状態を識別して表示する（「お互いにフォローしています」等）

**将来実装予定（Phase 5）:**
1. システムは、おすすめユーザー機能を提供する（興味タグベース）
2. システムは、フォロワーのフォロワー（2次つながり）を表示する
3. システムは、フォローリクエスト機能を提供する（プライベートアカウント用）
4. システムは、ブロック機能を提供する